...
class Bin(models.Model):
    class Meta:
        indexes = [models.Index(fields=["md5sum"])]
    ...
    def __str__(self):
        return " ".join(
            [str(x) for x in (self.md5sum, self.tax_id, self.comple, self.conta, self.strainhet)]
        )
...
class Enog(models.Model):
    class Meta:
        indexes = [models.Index(fields=["enog_name"])]

    enog_name = models.CharField(unique=True, max_length=32)
    enog_descr = models.TextField()

    def __str__(self):
        return "ID: {eid}\tDescription: {ed}".format(eid=self.enog_name, ed=self.enog_descr)
...
class PicaModel(models.Model):
    class Meta:
        ...
        indexes = [models.Index(fields=["model_name", "model_train_date"])]
...
class EnogRank(models.Model):
    ...
    def __str__(self):
        return "Enog ID: {eid}\tModel ID:" " {mid}\tRank {ir}\t(Model trained on {mtd})".format(
            eid=self.enog_id,
...
class PicaModelAccuracy(models.Model):
    class Meta:
        ...
        indexes = [models.Index(fields=["model", "comple", "conta"])]
    ...
    def __str__(self):
        return (
            "Name: {mid}\t type= {type} \t Completeness: {comple} \t "
            "Contamination {conta} \t bal. acc {balac}".format(
                mid=self.model.model_name,
...
class PicaModelTrainingData(models.Model):
    class Meta:
        ...
        indexes = [models.Index(fields=["model", "tax_id"])]
    ...
    def __str__(self):
        return "Taxid {ti} used in model {mod} (counted as {yn})".format(
            ti=self.tax_id, mod=self.model.model_name, yn=self.verdict
        )
...
class HmmerResult(models.Model):
    class Meta:
        ...
        indexes = [models.Index(fields=["enog", "bin"]), models.Index(fields=["bin"])]

    enog = models.ForeignKey(Enog, on_delete=models.CASCADE)
    bin = models.ForeignKey(Bin, on_delete=models.CASCADE)

    def __str__(self):
        return "Enog {eid} contained in the bin {mds}.".format(eid=self.enog_id, mds=self.bin_id)
...
class PicaResult(models.Model):
    class Meta:
        ...
        indexes = [models.Index(fields=["bin", "model"]), models.Index(fields=["bin"])]
    ...
    def __str__(self):
        return (
            "Bin {mds}: {v} ({acc} accuracy, "
            "{pval} pica_p_value) for model {mid} trained on {mtd}.".format(
                mds=self.bin.md5sum,
                v=str(self.verdict),
...
class PicaResultExplanation(models.Model):
    ...
    def __str__(self):
        return (
            "Bin {mds}, Model {mo}: presence={enog_is_present} of Enog {e}"
