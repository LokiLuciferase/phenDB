...
def sendinput(request):
    ...
    if postobj.get("example_data_button", None) is not None:
        ...
        postobj[
            "filename"
        ] = name  # this will return error if example data is used (if/else logic needed)
...
def getResults(request):
    ...
    try:
        ...
        context = {
            "errorMessage": "The requested page does not exist. Please note that all results are deleted after 30 days!"
        }
    ...
    context = {
        ...
        "progress": (job.finished_bins * 1.0 / job.total_bins) * 100
        if job.total_bins != 0
        else 0,  # necessary to avoid DivBy0 Exception
        ...
        "trait_counts_values": pica_result_for_ui.trait_counts.get_values()
        if pica_result_for_ui is not None
        else "",
        "trait_counts_titles": pica_result_for_ui.trait_counts.get_titles()
        if pica_result_for_ui is not None
        else "",
        "bin_summary_values": pica_result_for_ui.bin_summary.get_values()
        if pica_result_for_ui is not None
        else "",
        "bin_summary_titles": pica_result_for_ui.bin_summary.get_titles()
        if pica_result_for_ui is not None
        else "",
        "bin_alias_list": pica_result_for_ui.bin_alias_list
        if pica_result_for_ui is not None
        else "",
        "model_list": pica_result_for_ui.prediction.get_raw_title_list()
        if pica_result_for_ui is not None
        else "",
    }
...
def updateResultsAjax(request):
    ...
    key = getKeyFromUrl(request)
    job = Job.objects.get(key=key)
    pica_result_for_ui = PicaResultForUI(
        job=job,
        requested_conf=requested_conf,
        requested_balac=requested_balac,
        disable_cutoffs=disable_cutoffs,
    )
...
def serverError(request):
    context = {
        "errorMessage": "Unknown server error: This should not have happened. Please try again later or contact us!"
    }
    errorTemplate = loader.get_template("phenotypePredictionApp/error.xhtml")
    return HttpResponse(errorTemplate.render(context, request))
