...
parser.add_argument(
    ...
    type=argparse.FileType("r"),
    required=True,
    help="file containing the names in the order used in the database or the database (assuming .names.gz extension)",
)
...
parser.add_argument("-n", "--numthreads", default=1, type=int, help="number of parallel queries")

parser.add_argument(
    "-q", "--queriesperthread", default=10, type=int, help="queries per thread (default=10)"
)
...
parser.add_argument(
    "-p", "--port", default=51371, type=int, help="TCP port of the server (default=51371)"
)

parser.add_argument(
    "-f", "--finishtimeout", default=60, type=int, help="maximal seconds to finish (default=60)"
)

parser.add_argument(
    "-e", "--maxevalue", default=0.05, type=float, help="Maximal evalue to report (default=0.05)"
)
...
def init_dbnames(dbnamesfile, verbose):
    if os.path.isfile("%s.names.gz" % dbnamesfile):
        namesfilename = "%s.names.gz" % dbnamesfile
    elif dbnamesfile.endswith(".gz"):
        namesfilename = dbnamesfile
    else:
        sys.stderr.write(
            "Names file %s does not exist. Please extract all HMM or sequence names, e.g. by running:\n"
            % dbnamesfile
        )
        sys.stderr.write("grep '^ACC   ' (databasefile) | cut -c7- | gzip >%s" % (dbnamesfile))
        sys.exit(1)
...
def receive_hmmpgmd(s):
    ...
    if status:
        ...
        sys.stderr.write(
            "WARNING (trying another server with the same data): There was an error reported by HMMER: %s"
            % (errmsg[0].decode("utf-8", "ignore"))
        )
        return None
    else:
...
class serverPool:
    ...
    def report_down(self, server):
        if server in self.connections_by_server:
            sys.stderr.write(
                "WARNING: Server %s not responding. Trying other servers.\n" % (server)
            )
    ...
    def run(self):
        ...
        while not success:
            ...
            try:
                ...
                if self.verbose:
                    sys.stderr.write(
                        "Initializing connection to host %s at port %i.\n" % (server, self.port)
                    )
            ...
            for (queryname, querysequence) in self.queries:
                ...
                if not data:
                    ...
                    if self.verbose:
                        sys.stderr.write(
                            "Models: %i  Hits: %i  Dbsize: %i\n" % (nmodels, nhits, dbsize)
                        )
                    if nmodels != len(self.dbnames):
                        sys.stderr.write(
                            "FATAL: Number of models in database %i and number of database names from names file %i are different. Aborting.\n"
                            % (nmodels, len(self.dbnames))
                        )
                        sys.exit(1)
                    if not len(statusvalues) == 16:
                        sys.stderr.write(
                            "FATAL: Missmatch between the number of stats data elements recieved [%i] and expected [%i]"
                            % (len(statusvalues), 16)
                        )
                        sys.exit(1)
...
dbnames = init_dbnames(args.dbnames.name, args.verbose)
