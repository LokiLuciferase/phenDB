...
def queue_mock_jobs(number):
    ...
    for i in range(number):
        job = que.enqueue_call(func=mockjob, job_id=str(uuid4()), timeout="5h", ttl="5h")
...
def queue_failed_jobs(number):
    ...
    que = Queue("phenDB", connection=Redis())
    for i in range(number):
        job = que.enqueue_call(func=failedjob, job_id=str(uuid4()), timeout="5h", ttl="5h")
...
def queue_real_job(key):
    ...
    que = Queue("phenDB", connection=Redis())
    job = que.enqueue_call(func=mockjob, job_id=key, timeout="5h", ttl="5h")
    job.meta["ip"] = ("192.168.8.1",)
